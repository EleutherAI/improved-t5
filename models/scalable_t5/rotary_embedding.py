...
def apply_rotary_embedding(
  ...
  if decode and qlen == 1 and rotary_index is not None:
    ...
    qcos = jnp.expand_dims(cos[rotary_index, :], 0)
    qsin = jnp.expand_dims(sin[rotary_index, :], 0)
    # qcos, qsin: [batch, d]

    qcos = jax.lax.broadcast_in_dim(qcos, (batch, qlen, qheads, d), (0, 3))
    qsin = jax.lax.broadcast_in_dim(qsin, (batch, qlen, qheads, d), (0, 3))

    # qcos = jax.lax.broadcast_in_dim(qcos, (batch, qlen, qheads, d), (0))
    # qsin = jax.lax.broadcast_in_dim(qsin, (batch, qlen, qheads, d), (0))
